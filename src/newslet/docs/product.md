...
re-renders the latest stored email on every visit, so it is always exactly as
fresh as your newest issue, and costs nothing to keep that way.

:::

:::tier medium

`GET /` looks up the newest stored issue and re-renders it through the same
