# newslet — Product Guide

newslet is a personal daily news editor. Every morning it reads your feeds,
...
This guide explains what newslet does, feature by feature, and how each piece
...
Once a day, newslet sends you one email: a short, ranked list of stories it
...
Those taps are the only steering wheel newslet has, and they are enough.

At the bottom is a single link to your newslet homepage, a richer place to
...
configured value (`TO_EMAIL`); newslet is single-tenant by design.
...
newslet doesn't show you everything new. It shows you a ranked short list. The
...
The ranking is done by Claude, Anthropic's language model. newslet hands it
...
(`{"picks": [{url, title, blurb, source, score}]}`); on a parse failure newslet
...
## Telling newslet what you think
...
"more like this"), which gives newslet a stronger signal than a bare vote.
...
link newslet actually generated will be accepted — you can tap it from any
...
which you tell newslet what you're into. Plain language is fine: "machine
...
Underneath your own words, newslet keeps its own running notes: a "learned
...
After each email is sent, newslet looks back over a wider stretch of your
...
newslet pulls from several sources and ranks them all together, so the best
...
newsletters you've pointed at newslet. Each is covered below.

:::tier little

Whatever the source, a story has to be fresh (published in roughly the last
day) and new to you (not something newslet already showed you) to be
...
by pasting a feed URL (and an optional name); newslet checks each one every
...
morning newslet pulls the front pages and folds the stories into the same
...
newslet reads Hacker News through its search API rather than the bare RSS feed,
...
Beyond the sources you follow, newslet runs a fresh search of the open web each
...
newslet can subscribe to those for you and pull their links into the same daily
ranking.

In the admin console you give a newsletter a name and newslet mints a unique
email address for it. You paste that address into the newsletter's signup form.
From then on, every issue that newsletter sends lands at newslet, which pulls
...
email first, newslet clicks the confirmation link for you automatically.
...
`n-a8f3c2d1@your-mail-domain`). Mail sent to it is received by newslet,
...
At the bottom of the email, newslet sometimes suggests a source you don't
...
This is how your feed list grows on its own. newslet only suggests a source if
...
your current feeds. Before a suggestion is shown, newslet fetches the proposed
...
where you type a subject and newslet runs a fresh web search on it, returning a
...
It's a record of what newslet has been sending you, useful for finding a story
...
newslet is yours alone. Reaching the console, the homepage, or the archive
...
cryptographically signed, so only links newslet itself created will be accepted,
...
the issues newslet built for you.
...
newslet runs entirely as small, on-demand cloud functions rather than a server
...
*This guide is generated from newslet's own source code. The plain-language
