...
def _hit(oid: int, *, points: int, url: str | None = None, comments: int = 0,
         title: str | None = None, story_text: str = "") -> dict:
    return {
        "objectID": str(oid),
        ...
        "author": "tester",
        "created_at_i": 1_700_000_000 + oid,
...
def test_fetch_articles_sorts_by_points_and_caps():
    ...
    arts = hn.fetch_hn_articles(pages=2, fetch=_fake_fetch(pages), rank_cap=2)
...
def test_text_post_without_url_falls_back_to_thread():
    ...
    arts = hn.fetch_hn_articles(pages=1, fetch=_fake_fetch(pages))
    assert len(arts) == 1
...
def test_dedupes_across_pages():
    ...
    arts = hn.fetch_hn_articles(pages=2, fetch=_fake_fetch(pages))
...
def test_one_failing_page_does_not_lose_others():
    ...
    arts = hn.fetch_hn_articles(pages=3, fetch=flaky)
